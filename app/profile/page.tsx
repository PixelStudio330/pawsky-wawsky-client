...
import { LogOut, User, Mail, Image as ImageIcon, Sparkles, AlertCircle } from "lucide-react";
...
export default function ProfilePage() {
  ...
  // --- DETERMINE IF INPUT FIELDS ARE MUTATED ---
  const isFormChanged = 
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    const loadingToast = toast.loading("Updating your sanctuary metadata...", {
      style: {
        background: "#F4EBE1",
        color: "#4A3E3D",
        borderRadius: "1rem",
    ...
    try {
      ...
      setUpdating(false);
      const errTxt = err.response?.data?.message || "Failed to save profile updates. 🐾";
  ...
  return (
    <main className="min-h-screen bg-[#FBF8F3] flex items-center justify-center py-28 px-4 relative overflow-hidden selection:bg-[#F0A8A8]/30">
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        <FloatingEmoji emoji="🌸" delay={0} initialPos={{ top: "14%", left: "8%" }} />
        <FloatingEmoji emoji="🐾" delay={2.5} initialPos={{ bottom: "12%", right: "10%" }} rotateRange={25} />
        <FloatingEmoji emoji="🌿" delay={5} initialPos={{ top: "30%", right: "7%" }} rotateRange={-15} />
        <FloatingEmoji emoji="🎨" delay={7.5} initialPos={{ bottom: "25%", left: "6%" }} rotateRange={15} />
        ...
        >
          ...
          <div className="flex-1 space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-[#E8EFE9] border border-[#C4D7CB] text-[9px] uppercase tracking-widest font-black text-[#3B5443]">
              <Sparkles size={10} className="text-[#4E6E58]" /> **Sanctuary Core Resident**
            </div>
            <h1 className="text-3xl font-black text-[#3C3232] tracking-tight">{user.name}</h1>
            <p className="text-xs text-[#7A6A6A] font-semibold flex items-center justify-center md:justify-start gap-1.5 italic">
              <Mail size={13} className="text-[#4E6E58]" /> {user.email}
            </p>
          </div>
        </motion.div>

        {/* CORE DETAILS MATRIX CARD */}
        <motion.div
          variants={itemVariants}
        ...
        >
          ...
          <AnimatePresence>
            {message.text && (
              ...
              >
                <AlertCircle size={16} className="text-[#DB8B8B]" />
                {message.text}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="group">
                <label className="block text-[11px] font-black text-[#5C4D4D] uppercase tracking-wider mb-2.5 ml-1 flex items-center gap-1.5">
                  <User size={14} className="text-[#4E6E58]" /> **Full Name Identity**
                </label>
              ...
              {/* Locked System Email Input Column */}
              <div>
                <label className="block text-[11px] font-black text-[#5C4D4D] uppercase tracking-wider mb-2.5 ml-1 flex items-center gap-1.5 opacity-60">
                  <Mail size={14} className="text-[#A89898]" /> Locked Core Email
                </label>
            ...
            {/* Avatar URL Data Endpoint Row */}
            <div className="group">
              <label className="block text-[11px] font-black text-[#5C4D4D] uppercase tracking-wider mb-2.5 ml-1 flex items-center gap-1.5">
                <ImageIcon size={14} className="text-[#4E6E58]" /> **Avatar Photo Resource URL**
              </label>
              <div className="relative rounded-2xl bg-[#FFF]/80 border-2 border-[#EADFC9] group-focus-within:border-[#4E6E58] shadow-sm transition-all overflow-hidden">
                <input
                  type="url"
              ...
              <div className="flex w-full sm:w-auto items-center justify-end gap-4">
                {!isEditing ? (
                  ...
                  <>
                    ...
                    {/* --- CONDITIONALLY RENDER SAVE BUTTON ONLY IF MODIFIED --- */}
                    <AnimatePresence>
                      {isFormChanged && (
                        <motion.button
